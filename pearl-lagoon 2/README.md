...
- **2026-07-28**: Updated `next` and `eslint-config-next` from `15.0.3` to
  `15.5.22` (the current Maintenance LTS patch line) to resolve security
  advisories that Vercel's deploy-time scanner flags on `15.0.3`. This is a
  same-major-version update — no application code, routing, or config
  changes were needed. `react`/`react-dom` were left at `18.3.1`, which
  Next.js 15.5.x continues to support.
- No `package-lock.json` ships in this archive. Run `npm install` once
  after unzipping to generate an accurate one from the real npm registry —
  a hand-written lockfile isn't safe to ship (integrity hashes have to come
  from npm itself). Vercel will also generate one automatically on first
  deploy if none is committed.
